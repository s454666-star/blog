...
if (-not $env:JAVA_HOME) {
    $env:JAVA_HOME = $env:FOLDER_VIDEO_JAVA_HOME
}
if (-not $env:JAVA_HOME) {
    $env:JAVA_HOME = [Environment]::GetEnvironmentVariable('FOLDER_VIDEO_JAVA_HOME', 'User')
}
if (-not $env:JAVA_HOME) {
...
$unsignedApk = Join-Path $buildDir 'folder-video-tv-unsigned.apk'
$dexApk = Join-Path $buildDir 'folder-video-tv-with-dex.apk'
$alignedApk = Join-Path $buildDir 'folder-video-tv-aligned.apk'
$keystore = Join-Path $repoRoot 'storage\app\folder-video-tv.keystore'
$legacyBuildKeystore = Join-Path $buildDir 'debug.keystore'
$outputApk = Join-Path $repoRoot 'storage\app\folder-video-tv.apk'
...
$javaFiles = @(
    (Join-Path $generatedDir 'monster\mystar\foldervideotv\R.java'),
    (Join-Path $projectRoot 'app\src\main\java\monster\mystar\foldervideotv\MainActivity.java')
)
