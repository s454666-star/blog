...
Laravel provides a read-only NAS directory API and the list/viewer UI. Caddy serves video and image files directly from the configured `\\mc` shares. The Android back button closes the active viewer, then walks back through visited directories before exiting the app.
