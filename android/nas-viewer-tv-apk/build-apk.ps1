...
$unsignedApk = Join-Path $buildDir 'nas-viewer-tv-unsigned.apk'
$dexApk = Join-Path $buildDir 'nas-viewer-tv-with-dex.apk'
$alignedApk = Join-Path $buildDir 'nas-viewer-tv-aligned.apk'
$keystore = Join-Path $repoRoot 'storage\app\nas-viewer-tv.keystore'
$legacyBuildKeystore = Join-Path $buildDir 'debug.keystore'
$outputApk = Join-Path $repoRoot 'storage\app\nas-viewer-tv.apk'
...
$javaFiles = @(
    (Join-Path $generatedDir 'monster\mystar\nasviewertv\R.java'),
    (Join-Path $repoRoot 'android\shared-nas-direct\java\monster\mystar\shared\NasDirectBridge.java'),
    (Join-Path $projectRoot 'app\src\main\java\monster\mystar\nasviewertv\MainActivity.java')
)
