...
class ExtractVideoFeaturesCommand extends Command
{
     ...
     * php artisan video:extract-features --failed-only=1 --limit=100
     */
    protected $signature = 'video:extract-features
        ...
        {--failed-only=0 : 1=只重跑 video_features.last_error 有值的失敗資料；0=補缺 feature / 失敗 / 不完整資料}
        {--refresh : 已有特徵時也強制重建}';

    protected $description = '預設只補缺漏或失敗的影片特徵，不覆蓋已完成資料；設 --failed-only=1 可只跑明確失敗紀錄。';
    ...
    public function handle(VideoFeatureExtractionService $service): int
    {
        ...
        $query->orderBy('id')
            ->chunkById(200, function ($videos) use (
                $service,
                $applyFailedOnlyScope,
                $refresh,
                $limit,
                &$processed,
                &$skipped,
