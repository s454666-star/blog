...
class FetchTwFuturesHourlyPricesCommand extends Command
{
    ...
    public function handle(TwFuturesHourlyPriceFetcher $fetcher): int
    {
        ...
        if ($rows !== []) {
            ...
            $validationCounts = collect($rows)
                ->map(fn (array $row): string => (string) data_get($row, 'source_payload.validation.status', 'unverified'))
                ->countBy()
                ->sortKeys();
            if ($validationCounts->isNotEmpty()) {
                $this->line('驗證狀態：' . $validationCounts
