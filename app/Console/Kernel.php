...
class Kernel extends ConsoleKernel
{
    protected function schedule(Schedule $schedule): void
    {
        ...
        $schedule->command('crawler:85sugarbaby-import --headless --source=85sugarbaby_active_flow --limit=10 --age-min=18 --age-max=22 --areas=台北,新北')
            ->everyFifteenSeconds()
            ->name('crawler-85sugarbaby-import-15s')
            ->withoutOverlapping(1)
            ->runInBackground()
            ->appendOutputTo(storage_path('logs/crawler_85sugarbaby_import.log'))
            ->onFailure(function () {
                \Log::error('scheduler crawler:85sugarbaby-import 15s failed');
            });
