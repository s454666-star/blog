...
class NasViewerAppController extends Controller
{
    ...
    {
        $apkPath = (string) config('nas_viewer.tv_android_apk_path');
        $exists = is_file($apkPath);

        return response()->json(['data' => [
            'version_code' => (int) config('nas_viewer.tv_android_apk_version_code'),
            'version_name' => (string) config('nas_viewer.tv_android_apk_version_name'),
            'apk_url' => $this->publicUrl($request, route('nas-viewer-app.tv-android-apk', [], false)),
            'sha256' => $exists ? hash_file('sha256', $apkPath) : null,
            'size_bytes' => $exists ? filesize($apkPath) : null,
            'checked_at' => now()->toIso8601String(),
        ]])->header('Cache-Control', 'no-store, max-age=0');
    ...
    public function tvAndroidApk(): BinaryFileResponse|JsonResponse
    {
        ...
        return response()->download($apkPath, 'nas-viewer-tv.apk', [
            'Content-Type' => 'application/vnd.android.package-archive',
            'Cache-Control' => 'no-store, max-age=0',
        ]);
    }
