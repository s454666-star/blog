<?php
    ...
    class ReturnOrderController extends Controller
    {
        ...
        public function store(Request $request)
        {
            ...
            return response()->json(ReturnOrder::with(['order.deliveryAddress', 'orderItem.product', 'member.defaultDeliveryAddress'])->find($returnOrder->id), 201);
        }
        ...
        public function index(Request $request)
        {
            ...
            if ($user->role === 'admin') {
                $returnOrders = ReturnOrder::with(['order.deliveryAddress', 'orderItem.product', 'member.defaultDeliveryAddress'])->get();
            } else {
                $returnOrders = ReturnOrder::where('member_id', $user->id)
                    ->with(['order.deliveryAddress', 'orderItem.product', 'member.defaultDeliveryAddress'])
                    ->get();
            }
        ...
        public function update(Request $request, $id)
        {
            ...
            return response()->json(ReturnOrder::with(['order.deliveryAddress', 'orderItem.product', 'member.defaultDeliveryAddress'])->find($returnOrder->id), 200);
        }
