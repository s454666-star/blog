        ...
        return [
            'scan_group_tokens' => [
                ...
                'command_preview' => implode("\n", [
                    ...
                    '',
                    '[8002 PORT]',
                    'C:\\php\\php.exe artisan tg:dispatch-token-scan-items --done-action=delete --port=8002',
                    '',
                    '[8003 PORT]',
                ...
                'button_variants' => [
                    ...
                    [
                        'preset' => 'scan_group_tokens_port_8002',
                        'label' => '8002 PORT跑',
                        'title' => '掃群組 token：跑 8002',
                    ],
                    [
            ...
            'scan_group_tokens_port_8002' => [
                'hidden' => true,
                'eyebrow' => 'Telegram Scan',
                'title' => '掃群組 token：跑 8002',
                'summary' => '先掃描 token，再把待處理項目派送到 8002。',
                'steps' => [
                    [
                        'display' => 'C:\\php\\php.exe artisan tg:scan-group-tokens',
                        'command' => [self::PHP_BINARY, 'artisan', 'tg:scan-group-tokens'],
                    ...
                    [
                        'display' => 'C:\\php\\php.exe artisan tg:dispatch-token-scan-items --done-action=delete --port=8002',
                        'command' => [self::PHP_BINARY, 'artisan', 'tg:dispatch-token-scan-items', '--done-action=delete', '--port=8002'],
                    ],
                ],
            ],
            'scan_group_tokens_port_8003' => [
                'hidden' => true,
                'eyebrow' => 'Telegram Scan',
                'title' => '掃群組 token：跑 8003',
                'summary' => '先掃描 token，再把待處理項目派送到 8003。',
                'steps' => [
                    [
                        'display' => 'C:\\php\\php.exe artisan tg:scan-group-tokens',
                        'command' => [self::PHP_BINARY, 'artisan', 'tg:scan-group-tokens'],
                    ...
                    [
                        'display' => 'C:\\php\\php.exe artisan tg:dispatch-token-scan-items --done-action=delete --port=8003',
                        'command' => [self::PHP_BINARY, 'artisan', 'tg:dispatch-token-scan-items', '--done-action=delete', '--port=8003'],
                    ],
                ],
            ],
            'move_video_duplicates' => [
                ...
                'details' => '這組可以直接輸入要掃描的資料夾路徑。執行前會先同步 C:\\Users\\User\\Videos\\暫 底下的影片特徵 JSON，然後只拿你指定的掃描資料夾去比對 DB 與暫存索引；命中重複會直接刪檔，未命中則會搬進 C:\\Users\\User\\Videos\\暫 並同步更新 JSON，同時留下外部比對記錄。',
                'highlights' => [
                    '先比 DB，再比對 C:\\Users\\User\\Videos\\暫 的影片特徵索引。',
                    '不會處理 C:\\Users\\User\\Videos\\暫 自己裡面彼此是否重複。',
                    '命中重複會直接刪除原檔。',
                    '未命中重複會搬進 C:\\Users\\User\\Videos\\暫，並把特徵補進 JSON。',
                    '輸入資料夾後按 Enter 就可以直接開始跑。',
                ],
                'tags' => ['video_features', '自訂資料夾', '刪除重複'],
                'accent_from' => '#ff8fb1',
                'accent_to' => '#ffc29d',
            ...
            'dispatch_remaining_tokens' => [
                ...
                'button_variants' => [
                    ...
                    [
                        'preset' => 'dispatch_remaining_tokens_port_8002',
                        'label' => '8002 PORT跑',
                        'title' => '補跑剩餘 token：跑 8002',
                    ],
                    [
            ...
            'dispatch_remaining_tokens_port_8002' => [
                'hidden' => true,
                'eyebrow' => 'Backlog Flush',
                'title' => '補跑剩餘 token：跑 8002',
                'summary' => '把 token_scan_items 裡還沒處理完的項目送到 8002。',
                'steps' => [
                    [
                        'display' => 'C:\\php\\php.exe artisan tg:dispatch-token-scan-items --done-action=delete --port=8002',
                        'command' => [self::PHP_BINARY, 'artisan', 'tg:dispatch-token-scan-items', '--done-action=delete', '--port=8002'],
            ...
            'dispatch_remaining_tokens_port_8003' => [
                'hidden' => true,
                'eyebrow' => 'Backlog Flush',
                'title' => '補跑剩餘 token：跑 8003',
                'summary' => '把 token_scan_items 裡還沒處理完的項目送到 8003。',
                'steps' => [
                    [
                        'display' => 'C:\\php\\php.exe artisan tg:dispatch-token-scan-items --done-action=delete --port=8003',
                        'command' => [self::PHP_BINARY, 'artisan', 'tg:dispatch-token-scan-items', '--done-action=delete', '--port=8003'],
