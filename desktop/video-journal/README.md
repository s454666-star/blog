...
執行桌面的「美少女映像管」。在查詢頁任意位置拖入最多 50 部影片，即以原始檔名自動建立條目並返回查詢，不必按確認。也可點「新增映像」後點拖曳區多選檔案。不同資料夾、中文路徑與已連接的磁碟都會使用 Electron 取得的完整原始路徑；後端仍比對來源檔名、大小及抽樣指紋後整批新增。
...
打包輸出 `dist/FrameJournal-win32-x64/FrameJournal.exe`，需保留同資料夾內全部資源。安裝腳本複製至 `%LOCALAPPDATA%/Programs/FrameJournal` 並建立桌面捷徑「美少女映像管」。更新前先關閉程式；不需要管理員權限。這是供本機使用的未簽章版本。

`FrameJournal.exe --smoke` 隱藏啟動，僅讀取不匹配的測試搜尋頁，確認原生橋接、網站 DOM 和 Node 隔離；結果寫入 `%APPDATA%/frame-video-journal/smoke-result.json`，不讀取既有內容。
