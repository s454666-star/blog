...
- 查詢列表顯示標題與最多兩張自選封面；未設定則保留抽象卡片背景。封面以獨立延遲載入請求讀取，列表 HTML 不包含整篇圖文資料。
...
- 可選取最多 5 張大頭照，每張原圖最多 20 MB。前端先等比例縮入 1920×1080，後端再次驗證尺寸與格式，再存入本地 DB。可以替換、刪除、指定首張；第一張會另外顯示在列表卡片，文章封面另外自選，最多兩張。
...
編輯文章的「自選封面」可選擇最多 2 張圖片，支援縮圖預覽、替換、刪除與設為首張。每張原始圖片最多 20 MB，儲存前等比例縮至 1920×1080 範圍，不放大、不裁切；伺服器也會驗證與縮圖。文章、封面與大頭照請求合計上限 64 MB。

封面改為獨立選擇，不再使用內文第一張圖。沒有設定時顯示預設插畫；設定一張顯示一張，兩張則並排完整呈現。桌面查詢改為兩欄大型卡片，小螢幕改為單欄。標籤和第一張大頭照仍會顯示。
