...
RESOURCE_CODE_JSFILEEESBOT_PATTERN = re.compile(
    r"JSfileeesbot_[A-Za-z0-9_-]+",
    re.IGNORECASE,
)
...
RESOURCE_CODE_YYJMQG_PATTERN = re.compile(
    r"yyjmqg_bot_[A-Za-z0-9_-]+",
    re.IGNORECASE,
)
...
RESOURCE_CODE_JSCODEFILEBOT_PATTERN = re.compile(
    r"JScodefilebot_[A-Za-z0-9_-]+",
    re.IGNORECASE,
)
...
def _normalize_resource_code(raw_code: Any) -> Optional[str]:
    ...
    if RESOURCE_CODE_QQ_PATTERN.fullmatch(code):
        return code
    if RESOURCE_CODE_JSFILEEESBOT_PATTERN.fullmatch(code):
    ...
    if RESOURCE_CODE_YYJMQG_PATTERN.fullmatch(code):
        return "yyjmqg_bot_" + code[len("yyjmqg_bot_"):]
...
class DispatchPageCallbackRequest(BaseModel):
    bot_username: str
    message_id: int
    page_number: int
    response_timeout_seconds: float = 0.75
...
def _find_page_callback_button(message: Any, page_number: int) -> Optional[Tuple[str, str]]:
    wanted_page = int(page_number or 0)
    if wanted_page <= 0:
        return None

    forbidden_text = ("推送", "VIP", "收藏", "举报", "舉報")
    for row in list(getattr(message, "buttons", None) or []):
        for button in list(row or []):
            text = str(getattr(button, "text", "") or "").strip()
            if not text or any(keyword in text.upper() for keyword in forbidden_text):
                continue
            if _extract_page_target_number(text) != wanted_page:
                continue
...
@app.post("/bots/dispatch-page-callback")
async def dispatch_page_callback(payload: DispatchPageCallbackRequest) -> Dict[str, Any]:
    bot_username = str(payload.bot_username or "").strip()
    message_id = int(payload.message_id or 0)
    page_number = int(payload.page_number or 0)
    if not bot_username or message_id <= 0 or page_number <= 0:
        return {"status": "error", "reason": "invalid_request"}
    if not await _ensure_client_connected():
        return {"status": "error", "reason": "client_not_connected"}
    ...
    callback = _find_page_callback_button(message, page_number)
    if callback is None:
        return {
            "status": "error",
            "reason": "page_button_not_found",
            "message_id": message_id,
