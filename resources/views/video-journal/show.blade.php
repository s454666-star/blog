...
<section class="story-section" id="story" data-save-url="{{ route('video-journal.update', $entry->id) }}">
    ...
    <div class="story-main"><div class="story-toolbar"><span class="story-date">最後編輯 <span id="updated-at">{{ $entry->updated_at->format('Y.m.d H:i') }}</span></span><button id="edit-toggle" class="button secondary small" type="button">✎ 編輯小記</button></div>
        ...
        <section class="journal-metadata" aria-label="封面、標籤與大頭照">
            <div class="metadata-block" data-paste-kind="cover">
                ...
                <div id="cover-controls" hidden>
                    <div class="metadata-upload-row">
                        <button type="button" id="add-cover" class="button secondary small">▧ 選擇封面</button>
                        <div id="cover-paste-zone" class="image-paste-zone" tabindex="0" role="group" aria-label="封面貼上區">
                            <div class="image-paste-idle" id="cover-paste-idle"><span class="image-paste-icon">⎘</span><span>點此聚焦，可貼上後按 Enter 上傳</span></div>
                            <div id="cover-paste-staging" class="image-paste-staging" hidden></div>
                        </div>
                    </div>
                    <input type="file" id="cover-file" accept="image/png,image/jpeg,image/webp,image/gif" multiple hidden>
                    <p class="field-help">可自選 2 張，查詢時會一起顯示。每張最多 20 MB，自動等比例縮至最高 1080P。也可點貼上區，貼上後按 Enter 上傳。</p>
                </div>
            </div>
            ...
            <div class="metadata-block" data-paste-kind="portrait">
                ...
                <div id="portrait-controls" hidden>
                    <div class="metadata-upload-row">
                        <button type="button" id="add-portrait" class="button secondary small">▧ 加入大頭照</button>
                        <div id="portrait-paste-zone" class="image-paste-zone" tabindex="0" role="group" aria-label="大頭照貼上區">
                            <div class="image-paste-idle" id="portrait-paste-idle"><span class="image-paste-icon">⎘</span><span>點此聚焦，可貼上後按 Enter 上傳</span></div>
                            <div id="portrait-paste-staging" class="image-paste-staging" hidden></div>
                        </div>
                    </div>
                    <input type="file" id="portrait-file" accept="image/png,image/jpeg,image/webp,image/gif" multiple hidden>
                    <p class="field-help">最多 5 張，每張最多 20 MB；自動等比例縮至 1080P。第一張會顯示在查詢卡片，可指定其他照片。也可點貼上區，貼上後按 Enter 上傳。</p>
                </div>
            </div>
        </section>
