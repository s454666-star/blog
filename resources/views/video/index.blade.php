...
<script>
    ...
    function rebuildAndSortVideoList() {
        ...
        const $focused = $('.video-row.focused').first();
        if ($focused.length) {
            // 0ms 讓瀏覽器完成 reflow，再捲動
            setTimeout(() => {
    ...
    function focusMasterFace(videoId){
        ...
        const targetFace=$(`.master-face-img[data-video-id="${videoId}"]`);
        if(!targetFace.length) return;
        targetFace.addClass('focused');
           /* 改用自身容器捲動，100% 對齊 */
           const container=document.querySelector('.master-faces');
           container.scrollTo({
                   top: targetFace[0].offsetTop - container.clientHeight/2 + targetFace[0].clientHeight/2,
                   behavior:'smooth'
           });
    }
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.video-row', function (e) {
            $('.video-row').removeClass('focused');      // ① 先清除舊焦點
            $(this).addClass('focused');                // ② 自己加焦點
            const vid=$(this).data('id');
            focusMasterFace(vid);                       // ③ 通知右側同步
        ...
        function focusMaxIdVideo() {
            ...
            $('.video-row').removeClass('focused');
            $maxRow.addClass('focused');

            focusMasterFace(maxId);                     // 同步右側主面人臉
            $maxRow[0].scrollIntoView({                 // 捲到畫面中央
                behavior: 'smooth',
                block: 'center'
            });
        }
    ...
    const ro = new ResizeObserver(entries => {
        // 任何「聚焦列」尺寸變了，就把它重新拉回中央
        entries.forEach(entry => {
            if ($(entry.target).hasClass('focused')) {
    ...
    function watchFocusedRow() {
        ro.disconnect();                     // 先解除舊監聽
        const focused = document.querySelector('.video-row.focused');
        if (focused) ro.observe(focused);    // 只監聽當前那一列
    }

    // ------ 放在 script 最下方（與 ro 並存）------
    const listRO = new ResizeObserver(() => {
        ...
        if (Math.abs(rect.top + rect.height / 2 - vpCenter) > 10) {
