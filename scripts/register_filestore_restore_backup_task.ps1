...
Write-Host "Registering task $taskName to run daily at 03:00"
Register-ScheduledTask `
    -TaskName $taskName `
    -Action $action `
    -Trigger $trigger `
    -Settings $settings `
    -Description 'Back up telegram_filestore_sessions into telegram_filestore_restore_files every day at 03:00.' `
    -User $currentUser `
    -Force | Out-Null
