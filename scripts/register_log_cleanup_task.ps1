...
Write-Host "Registering task $taskName to run every 2 days at 03:00"
Register-ScheduledTask `
    -TaskName $taskName `
    -Action $action `
    -Trigger $trigger `
    -Settings $settings `
    -Description 'Delete all .log files under C:\www\blog and C:\Users\User\Pictures\train every 2 days.' `
    -User $currentUser `
    -Force | Out-Null
