...
class YuantaPortfolioControllerTest extends TestCase
{
    ...
    public function test_dashboard_renders_private_realtime_page(): void
    {
        ...
        $this->get(route('tw-stock.yuanta-portfolio.index', ['token' => 'test-token']))
            ...
            ->assertSee('const hasFuturesRealizedPnl = futuresRealizedYearPnl !== null && futuresRealizedYearPnl !== 0;', false)
            ->assertSee('futuresRealizedTarget.hidden = !hasFuturesRealizedPnl;', false)
            ->assertDontSee('* 期貨已實現：--')
            ...
            ->assertSee('data-pnl-wave="totalPnl"', false)
            ->assertSee('已實現＋未實現 · 近 60 個開市日')
            ->assertSee('data-total-pnl-history-level-line', false)
            ->assertSee("levelLine.setAttribute('y1', point.y.toFixed(2));", false)
            ->assertSee("levelLine.setAttribute('y2', point.y.toFixed(2));", false)
            ->assertSee('renderTotalPnlHistory', false)
            ->assertSee('unrealizedPnl + yearTotalPnl', false)
            ->assertSee('.slice(-60);', false)
            ->assertSee('setupTotalPnlHistoryHover', false)
            ...
            ->assertSee('const openingPrice = finiteNumber(points[0]?.open) ?? finiteNumber(points[0]?.value);', false)
            ->assertSee('baselineValue: openingPrice', false)
            ->assertDontSee('baselineValue: previousClose', false)
